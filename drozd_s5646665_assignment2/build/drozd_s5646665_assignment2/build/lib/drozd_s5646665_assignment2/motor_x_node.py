...
class MotorXNode(Node):
    ...
    def setpoint_callback(self, msg):
        target_x = msg.x
        self.pid.setpoint = target_x
        self.control_loop()

    #Control loop to reach the setpoint
    def control_loop(self):
        #Calculate the error
        error = abs(self.current_x - self.pid.setpoint)

        #Loop until error goes below the threshold
        while error > 0.01:
            control_signal = self.pid(self.current_x)
            self.current_x += control_signal
            self.motor_x_pub.publish(Float64(data=self.current_x))
